...
alignas(256) _Pragma("location=\"FLASH\"") static const uint8_t FeatherTraceFlash[256] = { 0 };
...
struct alignas(uint32_t) FaultDataFlashStruct {
    ...
    uint32_t version = 0;
    char marker2[8] = "Caused:";
    uint32_t cause;
    char marker3[8] = "My Bad:";
    uint32_t is_corrupted;
    char marker4[8] = "Fail #:";
    uint32_t failnum;
    char marker5[8] = "Line #:";
    int32_t line;
    char marker6[8] = "File n:";
    char file[64]; // may be corrupted if is_corrupted is true
    char marker7[8] = "Traced:";
...
static volatile unsigned saved_lr;
...
static void fill_phase2_vrs(volatile unsigned *fault_args)
{
    ...
    p_main_context.core.r[14] = fault_args[6]; // + 2;
    p_main_context.core.r[15] = fault_args[6];
    saved_lr = fault_args[5];
    p_main_context.core.r[13] = (unsigned)(fault_args + 8); // stack pointer
}
...
static _Unwind_Reason_Code trace_func(struct _Unwind_Context *context, void *arg)
{
    ...
    {
        if (myargs->strace_len == 1 && saved_lr != _Unwind_GetGR(context, 14))
        {
...
{
    // initialize the stack trace
    memset(arg, 0, sizeof(trace_arg_t));
...
void HandleFault() {
    ...
    if (static_cast<FeatherTrace::FaultType>(last_intr) == FeatherTrace::FaultType::WDTEarlyWarning) {
    ...
    // perform a "trace"
    // if we're in an interrupt context, we have to switch stacks to get the values we need
    // else we can just trace using the standard unwind method
    trace_arg_t arg;
    if (static_cast<FeatherTrace::FaultType>(last_intr) != FeatherTrace::FaultType::None)
...
void FeatherTrace::_Mark(const int line, const char* file) {
    ...
    // TODO: Fix
    // if (mem < 0 || mem > 60000)
    //     HandleFault(FeatherTrace::FAULT_OUTOFMEMORY);
}
